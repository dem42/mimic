...
impl MimicConfig {
    pub fn new() -> Result<Self> {
        ...
        let mut resource_base = current_exe.parent().ok_or(MimicCommonError::ExecutableBaseDirError)?;
        if let Some(dir_name) = resource_base.file_name() {
            if dir_name == "examples" {
                resource_base = resource_base.parent().ok_or(MimicCommonError::ExecutableBaseDirError)?;
            }
        }
        ...
        P: AsRef<Path>
    {
        ...
        if resolved.exists() {
            ...
            Err(MimicCommonError::ResourceFailedToResolve(resolved.as_os_str().to_owned()))
        }
    }
}