...
impl ShaderSource {    
    ...
    fn compile(&self, params: &ShaderCompileParams) -> Result<(), String> {
        println!("cargo:rerun-if-changed={}", self.shader_path.to_str().ok_or(String::from("Failed to get shader path"))?);

        let file_name = self.shader_path.file_name().ok_or("Failed to get file name")?;
        let extension = self.shader_path.extension().ok_or("Failed to get extension")?;
        ...
        let args = [self.shader_path.as_os_str(), &params.out_arg_flag, compiled_file_name.as_os_str()];

        let command_executable = if cfg!(target_os = "windows") { "glslc.exe" } else { "glslc" };
        let command_result = Command::new(command_executable)
                                    .args(&args)
                                    .output();
        println!("Running {:?} -> got result: {:?}", self.shader_path, command_result);
            ...
            Ok(output) => {
                if !output.status.success() {
                    match String::from_utf8(output.stderr) {
                        ...
                        Err(error) => Err(format!("Shader unknown compile error. Cannot convert to utf8: {:?}", error)),
                    }
                } else {
                    Ok(())
                }
...
fn main() {
    ...
    let shader_srcs = shader_compile_params.collect_shader_srcs().expect("Failed to collect shaders srcs");
