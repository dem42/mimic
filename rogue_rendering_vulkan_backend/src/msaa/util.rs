...
pub fn get_max_sample_count(physical_device_properties: vk::PhysicalDeviceProperties) -> vk::SampleCountFlags {
    ...
    for &flag in flags.iter() {
        if counts.contains(flag) {
            log!(Log::Info, "MSAA max samples count is {:?}", flag);
            return flag;
        }
    }
    
    log!(Log::Info, "MSAA max samples count is {:?}", vk::SampleCountFlags::TYPE_1);
