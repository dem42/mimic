...
impl SwapChainSupportDetails {
    ...
    fn choose_swap_extent(&self, window: &Window) -> Result<vk::Extent2D> {
        if self.capabilities.current_extent.width != u32::MAX {
            return Ok(self.capabilities.current_extent);
        } else {
            if let Some(current_monitor) = window.current_monitor() {
                ...
                Err(VulkanError::SwapExtentFailedToGetCurrentMonitor)
            }
        }
    }
...
impl SwapChainContainer {
    pub fn new(
        ...
        let extent = swap_chain_support_details.choose_swap_extent(window)?;
