...
impl QueueFamilyIndices {
    ...
    pub fn find(
        ...
        requirements: &DeviceRequirements) -> Result<Self> 
    {
        ...
        log!(Log::Debug, "Found queue data points: {:?}", queue_data_points);

        let mut remaining_queue_types: HashSet<_> = requirements.required_queues.iter().copied().collect();
        let mut indices: HashMap<u32, Vec<QueueType>> = HashMap::new();

        loop {
            let mut iter_failed = true;
            queue_data_points.sort_by(|a, b| {
                a.1.len().cmp(&b.1.len())
            });
...
pub fn create_queues(
    ...
    logical_device: &ash::Device) -> Result<HashMap<QueueType, ash::vk::Queue>>
{
    let mut queue_map = HashMap::new();
    for (&queue_family_index, queue_types_set) in queue_indices.indices.iter() {
        let QueueFamilyCreateData(queue_family_index, _, _) = QueueFamilyIndices::get_best_queue_family_data(queue_family_index);
        let queue = unsafe {
...
pub fn has_present_function(surface: &SurfaceContainer, physical_device: vk::PhysicalDevice, queue_family_index: u32) -> Result<bool> {
